...
public class GamePanel extends JPanel implements ActionListener, Runnable
{
	...
	{   
		...
		for(int i = 0; i < projectiles.size(); i++)
		{
			...
			if(p.getX() < -10 || p.getY() < - 10 || p.getX() > 1200 || p.getY() > 600)
			{
	...
	public void paintComponent(Graphics g)
	{
		...
		for(int i = 0; i < (Math.abs(player.getVY() + player.getVX())) + 3; i++)
		{
			int size = (int)(Math.random() * 6);

			int xVal = player.getX() - 40 + (int)(Math.random() * (hoverBoard.getWidth() - 10));
			int yVal = player.getY() + player.getHeight() + hoverBoard.getHeight() + (int)(Math.random() * 15);
